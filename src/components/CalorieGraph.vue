...
const dailyCalorieTotals = computed(() => {
  ...
  for (const entry of props.calorieEntries) {
    if (result.some((total) => isSameDay(total.date, entry.happenedAt))) {
      let matchingDailyTotal = result.filter((total) =>
        isSameDay(total.date, entry.happenedAt),
      )[0];
...
onMounted(() => {
  ...
  watch(props, (newProps, _) => {
    ...
    let limitAnnotation = chart.options.plugins?.annotation?.animations
      ?.limit as LineAnnotationOptions;
